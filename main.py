...
class MainApp(ctk.CTk):
    ...
    def show_main_menu(self):
        ...
        ctk.CTkButton(btn_frame, text="Order Manager", height=60, width=540, fg_color="#c27b1f", hover_color="#a86815", command=lambda: self.launch_module("Manager")).grid(row=2, column=0, columnspan=2, pady=10)
        ctk.CTkButton(btn_frame, text="Financial Dashboard", height=60, width=540, fg_color="#8e24aa", hover_color="#6a1b9a", command=lambda: self.launch_module("Finance")).grid(row=3, column=0, columnspan=2, pady=10)
        ctk.CTkButton(btn_frame, text="Search & Inventory View", height=60, width=540, fg_color="#1f538d", command=self.show_inventory_search).grid(row=4, column=0, columnspan=2, pady=10)
    ...
    def show_inventory_search(self):
        ...
        self.search_entry = ctk.CTkEntry(search_frame, width=600, height=40, placeholder_text="Search...")
        self.search_entry.grid(row=0, column=1, padx=10)
        self.search_entry.bind('<Return>', lambda e: self.perform_combined_search())
        ctk.CTkButton(search_frame, text="SEARCH", width=150, height=40, command=self.perform_combined_search).grid(row=0, column=2, padx=10)
    ...
    def perform_combined_search(self):
        ...
        if doc.exists:
            ...
            return
        docs = db.collection("products").where(filter=FieldFilter("name_lower", ">=", query)).where(filter=FieldFilter("name_lower", "<=", query + "\uf8ff")).limit(15).get()
        if not docs:
            ctk.CTkLabel(self.results_panel, text="No matches found.", text_color="#bbbbbb", font=("Arial", 16)).pack(pady=40)
            return
        for d in docs:
            p_data = d.to_dict()
            ctk.CTkButton(self.results_panel, text=f"{p_data.get('name')} | {p_data.get('shade')}", anchor="w", height=45, fg_color="#333", command=lambda bar=d.id, data=p_data: self.display_item_stock(bar, data)).pack(fill="x", pady=3, padx=20)
    ...
    def display_item_stock(self, barcode, prod_data):
        ...
        ctk.CTkLabel(header_frame, text=f"Shade: {prod_data.get('shade')} | Barcode: {barcode}", text_color="gray").pack(pady=(5,10), padx=20, anchor="w")
        ...
        if not active_batches:
            ...
            for b_data in active_batches:
                ts = b_data.get('timestamp'); ds = ts.strftime("%d/%m/%Y") if ts else "N/A"
                card = ctk.CTkFrame(self.results_panel, fg_color="#333333")

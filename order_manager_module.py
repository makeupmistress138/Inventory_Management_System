...
class OrderManagerModule(ctk.CTkFrame):
    ...
    def render_order_details(self, order_id, data):
        ...
        info_text = f"Phone: {data.get('customer_phone')}  |  Address: {data.get('address')}\n" \
                    f"Channel: {data.get('channel')}  |  Shipment Option: {data.get('shipment_option')}\n" \
                    f"Shipment ID: {data.get('shipment_id', 'N/A')}  |  Notes: {data.get('notes', 'None')}\n" \
        ...
        # CHECK IF LOCKED (Completed/Cancelled/Returned)
        ...
        combo = ctk.CTkComboBox(stat_f, values=["PENDING", "CONFIRMED", "SHIPPED", "DELIVERED"], variable=self.status_var, width=150)
        ...
        # Financial / Cancel Row (STATE MACHINE LOGIC)
        ...
        btn_return = ctk.CTkButton(bot_f, text="RETURN / LOSS", fg_color="#c27b1f", command=self.process_return)
        ...
        if is_locked:
            ...
            # Confirmed/Shipped/Delivered: Cannot Cancel, must Return or Complete
            btn_cancel.configure(state="disabled")
    ...
    def draw_edit_cart(self, is_locked=False):
        ...
        for barcode, item in self.edit_cart.items():
            ...
            item_text = f"{item['name']} | {item['shade']} @ {item['selling_price']} EGP"
            ctk.CTkLabel(f, text=item_text, justify="left").pack(side="left", padx=15)
    ...
    def show_suggestion_popup(self, docs):
        ...
        for d in docs:
            ...
            btn = ctk.CTkButton(scroll, text=f"{p_data.get('name')} | {p_data.get('shade')}", anchor="w", fg_color="#333",
    ...
    def select_from_popup(self, barcode, p_data, popup):
        self.add_to_edit_dict(barcode, p_data)
        popup.destroy()

    def add_to_edit_dict(self, barcode, data):
        if barcode in self.edit_cart:
            self.edit_cart[barcode]['qty'] += 1
        else:
            self.edit_cart[barcode] = {"name": data.get('name'), "shade": data.get('shade'), "qty": 1, "selling_price": float(data.get('selling_price', 0.0))}
        self.draw_edit_cart(False)
        ...
        try:
            ...
            for barcode, cart_item in self.edit_cart.items():
                ...
                if needed > held:
                    ...
                    if avail < extra_needed:
                        raise ValueError(f"Not enough stock for {cart_item['name']}. Need {extra_needed} more, but only have {avail}.")
            ...
            self.controller.show_error_popup(f"Failed to update: {str(e)}")
    ...
    def refresh_current_order(self):
        ...
        if doc.exists:
            self.render_order_details(doc.id, doc.to_dict())
    ...
    def process_return(self):
        ...
        ctk.CTkLabel(popup, text="Order Returned. Enter incurred losses (EGP):").pack(pady=10)
        ctk.CTkLabel(popup, text="(Enter 0 if there are no courier fees/losses)", text_color="gray").pack()
        loss_var = ctk.StringVar(value="0")
        ent = ctk.CTkEntry(popup, textvariable=loss_var, justify="center")
        ent.pack(pady=10)

        def confirm():
            try:
                loss = float(loss_var.get())
                for item in items:
                    b_id = item.get('batch_id')
                    if b_id: db.collection("batches").document(b_id).update({"qty_remaining": firestore.Increment(int(item.get('qty', 0)))})
                
                # Accrual return logic: Status RETURNED, payout is strictly negative loss.
                db.collection("sales_orders").document(self.selected_order_id).update({"status": "RETURNED", "payout_received": -loss})
                popup.destroy()
                self.controller.show_error_popup(f"Returned. -{loss} EGP loss logged.")
                self.refresh_current_order()
            except ValueError: pass

        ctk.CTkButton(popup, text="RESTORE STOCK & LOG LOSS", fg_color="#c27b1f", command=confirm).pack(pady=10)
    ...
    def trigger_completion(self):
        ...
        # SMART MATH: Default to Total - Deposit
        ...
        ent = ctk.CTkEntry(popup, textvariable=payout_var, justify="center")
        ent.pack(pady=10)
